...
# renders for both), one at /api/comments (moderation, cross-media, flat --
# mirrors the admin "Comments" page, which lists every comment in the
# gallery regardless of which photo/video it's on).
...
async def create_video_comment(
    ...
    parent: Comment | None = None
    if payload.parent_id:
        result = await db.execute(select(Comment).where(Comment.id == payload.parent_id, Comment.video_id == video_id))
        parent = result.scalar_one_or_none()
        if parent is None:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "Invalid parent comment")
    ...
    if parent is not None and parent.author_id and parent.author_id != comment.author_id:
        ...
        await create_notification(
            ...
            user_id=parent.author_id,
            type="comment_reply",
            message=f"{replier_name} replied to your comment on {video_title}",
            href=f"/video/{video_id}",
        )
...
async def list_all_comments(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Comment, Photo.title, Video.title)
        .outerjoin(Photo, Photo.id == Comment.photo_id)
        .outerjoin(Video, Video.id == Comment.video_id)
    ...
    return [
        ...
        for comment, photo_title, video_title in rows
    ]
