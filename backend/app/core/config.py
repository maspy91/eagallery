...
#   
    ...
    # gemini-3.5-flash is the current stable (non-preview) Flash tier as
    # of when this was written -- a single named setting rather than
    # hardcoding the model string in app/core/ai.py so it can be bumped
    # (or pinned to a specific dated snapshot) without a code change.
    # Deliberately NOT a bare "-latest" alias in production: Google can
    # swap what a "-latest" alias points at without notice, which would
    # silently change response behavior/cost for both AI features here.
    AI_MODEL: str = "gemini-3.5-flash"
    # Caps a single reply's length -- protects both latency (the chat
    # widget is a synchronous request/response, not streamed) and cost,
    # since Gemini is billed per output token.
