<script lang="ts">
	...
	import { KeyRound, Lock, Eye, EyeOff, CircleCheckBig } from '@lucide/svelte';
...
<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-16">
	<div class="w-full max-w-md animate-fade-in">
		<div class="text-center mb-8">
			...
			<h1 class="text-3xl font-bold text-foreground mb-2">Set a new password</h1>
			<p class="text-muted-foreground">Choose a strong password you haven't used before</p>
		</div>
		...
		<div class="glass elevated rounded-2xl p-8">
			{#if !token}
				<p class="text-center text-sm text-destructive">
					...
					<a href="/forgot-password" class="underline">forgot password</a> page.
				</p>
			{:else if done}
				<div class="text-center space-y-4 animate-fade-in">
					...
					<div>
						<p class="font-semibold text-foreground">Password updated</p>
						<p class="text-sm text-muted-foreground mt-1">You can now sign in with your new password.</p>
					</div>
					<div class="flex gap-3 justify-center pt-2">
						<a
							href="/login"
							class="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity"
						>
							Customer sign in
						</a>
						<a
							href="/auth"
							class="px-4 py-2 rounded-lg glass border-border/60 text-sm font-medium hover:border-primary/50 transition-smooth"
						>
							Admin sign in
						</a>
					</div>
